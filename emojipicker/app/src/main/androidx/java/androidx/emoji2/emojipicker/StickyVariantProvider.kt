...
internal class StickyVariantProvider(context: Context) {
    ...
    private val sharedPreferences = if (userUnlocked) {
        context.getSharedPreferences(PREFERENCES_FILE_NAME, MODE_PRIVATE)}else{null}
    ...
    internal fun update(baseEmoji: String, variantClicked: String) {
        stickyVariantMap.apply {
            ...
            sharedPreferences?.edit()?.putString(STICKY_VARIANT_PROVIDER_KEY, entries.joinToString(ENTRY_DELIMITER))?.commit()
        }
    }
}
